...
class FinetuningDataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		#print("mol:",mol)
		#print("smiles_bert_adjmat:",smiles_bert_adjmat,smiles_bert_adjmat.size)

		#print("smiles_bert_adjmat1111111111111", smiles_bert_adjmat,smiles_bert_adjmat.size)
...
def main():
	# 加载超参数
	with open('D:\\DATA\\python_workspace\\CHEM-BERT-master\\CHEM-BERT-master\\ft_service\\input\\input.json', 'r') as f:
	...
	params = {'batch_size':2, 'dropout':0, 'learning_rate':0.00001, 'epoch':15, 'optimizer':'Adam', 'model':'Transformer'}

	Smiles_vocab = Vocab()
	# read data
	dataset = FinetuningDataset('D:\\DATA\\python_workspace\\CHEM-BERT-master\\CHEM-BERT-master\\ft_service\\input\\clintox01.csv', Smiles_vocab, seq_len=256)
	...
	model.load_state_dict(torch.load('D:\\DATA\\python_workspace\\CHEM-BERT-master\\CHEM-BERT-master\\model\\pretrained_model.pt', map_location=device))
	...
	for epoch in range(params['epoch']):
		...
		for i, data in enumerate(train_dataloader):
			...
			#print("smiles_bert_input:", data["smiles_bert_input"], data["smiles_bert_input"].shape)
			#print("position_num:", position_num, position_num.shape)
			#print("adj_mask:", data["smiles_bert_adj_mask"], data["smiles_bert_adj_mask"].shape)
			#print("adj_mat:",data["smiles_bert_adjmat"],data["smiles_bert_adjmat"].shape)
			...
			loss = criterion(output, data["smiles_bert_label"].float())
			optim.zero_grad()
			loss.backward()
			optim.step()
		...
		with torch.no_grad():
			...
			for i, data in enumerate(valid_dataloader):
				...
				output = model.forward(data["smiles_bert_input"], position_num, adj_mask=data["smiles_bert_adj_mask"], adj_mat=data["smiles_bert_adjmat"])
				output = output[:,0]
				valid_loss = criterion(output, data["smiles_bert_label"].float())
		...
		end_time = time.time()
		print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(start_time)),time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(end_time)),"epoch:",epoch)
	...
	output_csv['test_score_list'] = test_score_list
